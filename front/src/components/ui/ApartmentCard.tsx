...
import { Card, CardContent, Typography, Button, Chip, Box, TextField } from '@mui/material';
...
interface ApartmentCardProps {
  ...
  onCancel?: (apartment: Apartment) => void;
...
export const ApartmentCard: React.FC<ApartmentCardProps> = ({
  ...
  const handleSell = () => {
    if (onSell && isOwner && !apartment.isForSale && price) {
      const apartmentWithPrice = { ...apartment, price: BigInt(parseFloat(price) * 1e18) };
      onSell(apartmentWithPrice);
    }
  };

  const handleCancel = () => {
    if (onCancel && isOwner && apartment.isForSale) {
      onCancel(apartment);
    }
  };
